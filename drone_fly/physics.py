...
def model_xml(course: Course) -> str:
    ...
    for i, g in enumerate(course.gates):
        for side in (-1, 1):
            geoms.append(f'<geom name="gate_{i}_side_{side}" type="box" '
                         f'pos="{g.x} {g.y + side * (g.width / 2 + .05)} {g.z}" '
                         f'size=".06 .05 {g.height / 2 + .1}" rgba=".95 .19 .06 1"/>')
            geoms.append(f'<geom name="gate_{i}_bar_{side}" type="box" '
                         f'pos="{g.x} {g.y} {g.z + side * (g.height / 2 + .05)}" '
                         f'size=".06 {g.width / 2} .05" rgba=".95 .19 .06 1"/>')
    for i, o in enumerate(course.obstacles):
        geoms.append(f'<geom name="pillar_{i}" type="cylinder" pos="{o.x} {o.y} {o.z}" '
                     f'size="{o.radius} {o.height / 2}" rgba=".28 .32 .35 1"/>')
    sites, actuators = [], []
    for i, (x, y, spin) in enumerate(((ARM, ARM, 1), (-ARM, ARM, -1),
                                      (-ARM, -ARM, 1), (ARM, -ARM, -1))):
        sites.append(f'<site name="rotor{i}" pos="{x} {y} 0" size=".025"/>')
        actuators.append(f'<general name="motor{i}" site="rotor{i}" '
                         f'gear="0 0 1 0 0 {spin * .015}" ctrllimited="true" ctrlrange="0 2.4"/>')
...
class Flight:
    def __init__(self, seed=42, difficulty="standard", render=True):
        ...
        self.gate_geoms = [[self.model.geom(f"gate_{i}_{kind}_{side}").id
                           for kind in ("side", "bar") for side in (-1, 1)]
                          for i in range(len(self.course.gates))]
        self.mix = np.array([[1, 1, 1, 1], [ARM, ARM, -ARM, -ARM],
    ...
    def _colors(self):
        for i, ids in enumerate(self.gate_geoms):
            self.model.geom_rgba[ids] = ([.95, .19, .06, 1] if i == self.gate_index
                                         else [.24, .31, .34, 1])
    ...
    def observe(self):
        ...
        self.model.cam_pos[self.camera_id] = self.position + [.2, 0, .025]
        mujoco.mj_forward(self.model, self.data)
    ...
    def step(self, command):
        ...
        if self.status != "flying":
            return
        self.command = np.clip(command, [-.5, -2, -1.5], [2, 2, 1.5])
        ...
        for _ in range(12):
            ...
            err_matrix = .5 * (r_des.T @ rotation - rotation.T @ r_des)
            err = np.array([err_matrix[2, 1], err_matrix[0, 2], err_matrix[1, 0]])
            torque = -.09 * err - .018 * self.data.qvel[3:6]
        ...
        if self.gate_index < len(self.course.gates):
            ...
            if previous[0] < gate.x <= now[0]:
                ...
                if abs(crossing[1] - gate.y) < gate.width / 2 - BODY_RADIUS and \
                        abs(crossing[2] - gate.z) < gate.height / 2 - BODY_RADIUS:
                    self.passed += 1
                else:
                    self.missed += 1
                self.gate_index += 1
        ...
        if self.status == "flying":
            if self.gate_index == len(self.course.gates):
                ...
                self.status = "timeout"
            elif now[2] > 5 or now[2] < .08 or abs(now[1]) > 5 or now[0] < -2:
                self.status = "out-of-bounds"

    def snapshot(self):
        return {"time": float(self.data.time), "position": self.position.tolist(),
                "quaternion": self.data.qpos[3:7].tolist(), "velocity": self.velocity.tolist(),
                "motors": self.data.ctrl.tolist(), "command": self.command.tolist(),
                "gateIndex": self.gate_index, "passed": self.passed, "missed": self.missed,
                "status": self.status, "collision": self.collision, "distance": self.distance,
                "energy": self.energy}
